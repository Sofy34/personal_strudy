...
def get_and_save_sent_vectors(dir_name, doc_idx, ft, dim=300):
    sent_db = pd.read_csv(os.path.join(
        os.getcwd(), defines.PATH_TO_DFS, dir_name, "{:02d}_sent_db.csv".format(doc_idx)))
    sent_vec_db = get_vector_per_sentence(sent_db, ft, dim)
    sent_vec_db.to_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS, dir_name,
                       "{:02d}_sent_vec{}_db.csv".format(doc_idx, dim)), index=False)
...
def get_vector_per_sentence(db, ft, dim=300):
    ...
    sent_vectors = [ft.get_sentence_vector(
        row['text']) for index, row in db.iterrows()]
...
def get_and_save_doc_similarity(dir_name, doc_idx, dim=300):
    sent_vec_db = pd.read_csv(os.path.join(os.getcwd(
    ), defines.PATH_TO_DFS, dir_name, "{:02d}_sent_vec{}_db.csv".format(doc_idx, dim)))
    sim_db = pd.DataFrame(cosine_similarity(sent_vec_db))
    sim_db.to_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS, dir_name,
                  "{:02d}_sent_sim_vec{}_db.csv".format(doc_idx, dim)), index=False)
...
def get_stop_words_rate(doc_idx):
    sent_db = pd.read_csv(os.path.join(
        os.getcwd(), defines.PATH_TO_DFS, "{:02d}_sent_db.csv".format(doc_idx)))
...
def get_all_docs_lemma(dir_name):
    all_docs_lemma = pd.concat(map(pd.read_csv, glob.glob(os.path.join(
        os.getcwd(), defines.PATH_TO_DFS, dir_name, "*_sent_lemma_db.csv"))), axis=0)
...
def tfidf_transform_doc(dir_name, doc_idx, tfidf, per_lemma=True):
    if per_lemma:
        sent_lemma_db = pd.read_csv(os.path.join(os.getcwd(
        ), defines.PATH_TO_DFS, dir_name, "{:02d}_sent_lemma_db.csv".format(doc_idx)), usecols=['sent_lemma'])
        corpus = sent_lemma_db['sent_lemma'].tolist()
    else:
        sent_db = pd.read_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS,
                              dir_name, "{:02d}_sent_db.csv".format(doc_idx)), usecols=['text'])
...
def tfidf_build_all_save_per_doc(dir_name, per_word=True, per_lemma=True, analyzer='char', tf_suffix='', stop_words=[], doc_indices=[]):
    tf = tfidf_fit(dir_name=dir_name, per_word=per_word,
                   per_lemma=per_lemma, analyzer=analyzer, stop_words=stop_words)
    ...
    if len(doc_indices) == 0:
        sent_lemma_db_list = glob.glob(os.path.join(
            os.getcwd(), defines.PATH_TO_DFS, dir_name, "*_sent_lemma_db.csv"))
        doc_indices = [common_utils.get_doc_idx_from_name(
            doc_name) for doc_name in sent_lemma_db_list]
...
def get_and_save_sent_lemma_db(dir_name, doc_idx):
    doc_name = os.path.join(os.getcwd(), defines.PATH_TO_DFS,
                            dir_name, "{:02d}_sent_pos_db.csv".format(doc_idx))
    ...
    sent_lemma_db['sent_lemma'] = sent_pos_db.groupby(
        'sent_idx')['LEMMA'].apply(lambda x: "%s" % ' '.join(x)).tolist()
    sent_lemma_db.to_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS,
                         dir_name, "{:02d}_sent_lemma_db.csv".format(doc_idx)), index=False)
    print("{} sent lemma db saved".format(doc_idx))


def get_and_save_sent_pos_count_db(dir_name, doc_idx):
    columns_to_count = ['POSTAG', 'f_gen', 'f_num',
                        'f_suf_gen', 'f_suf_num', 'f_suf_per', 'f_per', 'f_tense']
    sent_pos_db = pd.read_csv(os.path.join(os.getcwd(
    ), defines.PATH_TO_DFS, dir_name, "{:02d}_sent_pos_db.csv".format(doc_idx)))
    ...
    count_db.to_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS, dir_name,
                    "{:02d}_sent_pos_count_db.csv".format(doc_idx)), index=False)
...
def merge_sent_pos_db(dir_name, doc_idx):
    sent_db = pd.read_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS, dir_name,
                          "{:02d}_sent_db.csv".format(doc_idx)), usecols=defines.SENT_FEATURES)
    count_db = pd.read_csv(os.path.join(os.getcwd(
    ), defines.PATH_TO_DFS, dir_name, "{:02d}_sent_pos_count_db.csv".format(doc_idx)))
    merged_db = pd.merge(sent_db, count_db, left_index=True,
                         right_index=True, validate="one_to_one")
    merged_db.to_csv(os.path.join(os.getcwd(), defines.PATH_TO_DFS,
                     dir_name, "{:02d}_merged_db.csv".format(doc_idx)), index=False)
...
def load_doc_features(dir_name, doc_idx, tf_types=['word', 'char_wb', 'lemma'], merged_str="merged_db", tf_suffix=''):
    global curr_doc_db
    curr_doc_db['merged'] = pd.read_csv(os.path.join(os.getcwd(
    ), defines.PATH_TO_DFS, dir_name, "{:02d}_{}.csv".format(doc_idx, merged_str)))
    curr_doc_db['sim_vec'] = pd.read_csv(os.path.join(os.getcwd(
    ), defines.PATH_TO_DFS, dir_name, "{:02d}_sent_sim_vec300_db.csv".format(doc_idx)))
    for tf_type in tf_types:
        if 'char' in tf_type:
            tf_suffix = ''
        curr_doc_db['tfidf_{}'.format(tf_type)] = sparse.load_npz(os.path.join(os.getcwd(
        ), defines.PATH_TO_DFS, dir_name, "{:02d}_tfidf_{}{}.npz".format(doc_idx, tf_type, tf_suffix)))
    return curr_doc_db


def save_doc_packed_features(doc_idx, dictionary_data):
    j_file = open(os.path.join(os.getcwd(), defines.PATH_TO_DFS,
                  "{:02d}_packed_dict.csv".format(doc_idx)), "w")
...
def pack_doc_features(doc_idx, seq_len=6, step=6):
    # TBD take last indices as well
    X_doc = [doc2features(first_sent_idx)
             for first_sent_idx in curr_doc_db['merged'].index[:-seq_len:step]]
    # save_doc_packed_features(doc_idx,X_doc) #TBD open after solve 'Object of type int64 is not JSON serializable'
    y_doc = [doc2labels(first_sent_idx)
             for first_sent_idx in curr_doc_db['merged'].index[:-seq_len:step]]
...
def pack_all_doc_features(seq_len=6, step=6):
    ...
    merged_db_list = glob.glob(os.path.join(
        os.getcwd(), defines.PATH_TO_DFS, "*_merged_db.csv"))
    merged_db_list.sort()
...
def pack_doc_sentences(doc_idx, neighbor_radius):
    ...
    X_doc = [sent2features(sent_idx, sent_idx, doc_len, neighbor_radius)
             for sent_idx in curr_doc_db['merged'].index]
    # save_doc_packed_features(doc_idx,X_doc) #TBD open after solve 'Object of type int64 is not JSON serializable'
...
def reshape_doc_paragraphs_to_sequence(X, y, doc_idx, seq_len, step):
    X_seq = [list(itertools.chain.from_iterable(X[i: i+seq_len]))
             for i in np.arange(0, len(X), step)]
    y_seq = [list(itertools.chain.from_iterable(y[i: i+seq_len]))
             for i in np.arange(0, len(y), step)]
    groups_seq = [doc_idx for i in range(len(y_seq))]
...
def pack_doc_per_paragraph(doc_idx, doc_as_sequence, neighbor_radius):
    ...
    X_doc = [par2features(par_idx, doc_as_sequence, neighbor_radius)
...
def par2features(par_idx, doc_as_sequence, neighbor_radius):  # doc_len is dummy variable
    par_len = curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").shape[0]
    par_sentences = curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").index
    doc_sent_num = len(curr_doc_db['merged'].index)
    # print("par {} has {} sentenses".format(par_idx,par_len))
    if doc_as_sequence:
        X_par = [sent2features(sent_idx, sent_idx, doc_sent_num,
                               neighbor_radius) for sent_idx in par_sentences]
    else:
        X_par = [sent2features(sent_idx, idx_in_seq, par_len, neighbor_radius)
                 for idx_in_seq, sent_idx in enumerate(par_sentences)]
    return X_par


# 0 means no limit
def pack_doc_per_paragraph_limit(doc_idx, limit, doc_as_sequence, neighbor_radius):
    par_indices = curr_doc_db['merged']['par_idx_in_doc'].unique()
    X_doc = []
    y_doc = []
    groups_doc = []
    for par_idx in par_indices:
        sub_par_x_list = par2features_limit(
            par_idx, limit, doc_as_sequence, neighbor_radius)
...
def reshape_docs_map_to_seq(docs_map, per_par, seq_len, step):
    for doc in docs_map.keys():
        if per_par:
            X_doc, y_doc, groups_doc = reshape_doc_paragraphs_to_sequence(
                docs_map[doc]['X'], docs_map[doc]['y'], doc, seq_len, step)
        else:
            X_doc, y_doc, groups_doc = reshape_doc_features_to_sequence(
                docs_map[doc]['X'], docs_map[doc]['y'], doc, seq_len, step)
...
def par2label_limit(par_idx, limit=0):
    par_len = curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").shape[0]
    sent_indices = curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").index
    y_par = []
...
# limit maximum sentences per paragraph
def par2features_limit(par_idx, limit, doc_as_sequence, neighbor_radius):
    par_len = curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").shape[0]
    sent_indices = curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").index
    ...
    for i in np.arange(0, par_len, limit):  # 0-7, 8-15....
        sub_indices = sent_indices[i:min(par_len, i+limit)]
        if doc_as_sequence:
            sub_par = [sent2features(
                sent_idx, sent_idx, doc_sent_num, neighbor_radius) for sent_idx in sub_indices]
        else:
            sub_par = [sent2features(sent_idx, idx, len(
                sub_indices), neighbor_radius) for idx, sent_idx in enumerate(sub_indices)]
...
def doc_sent2features_db(dox_idx):
    ...
    for sent_idx in sent_indices:
        doc_features_db[sent_idx] = sent2features(
            sent_idx, sent_idx, sent_count)


def par2label(par_idx):
    y_par = [sent2label(sent_idx) for sent_idx in curr_doc_db['merged'].query(
        "par_idx_in_doc == @par_idx").index]
    return y_par
...
def pack_reshape_all_doc_sentences(seq_len, step, per_par=False):
    ...
    for doc_idx in doc_db.doc_idx_from_name:
        ...
        if per_par:
            ...
            X_seq, y_seq, groups_seq = reshape_doc_paragraphs_to_sequence(
                X_doc, y_doc, doc_idx, seq_len, step)
        else:
            X_doc, y_doc, groups_doc = pack_doc_sentences(doc_idx)
            X_seq, y_seq, groups_seq = reshape_doc_features_to_sequence(
                X_doc, y_doc, groups_doc, seq_len, step)
        X.extend(X_seq)
        y.extend(y_seq)
...
def pack_all_doc_sentences(per_par=False, tf_types=['word', 'char_wb']):
    ...
    groups = []
    sent_lemma_db_list = glob.glob(os.path.join(
        os.getcwd(), defines.PATH_TO_DFS, "*_sent_lemma_db.csv"))
    # doc_db_path = os.path.join(os.getcwd(),defines.PATH_TO_DFS,"doc_db.csv")
    # if os.path.isfile(doc_db_path):
    #     doc_db = pd.read_csv(doc_db_path)
    ...
    print("{} sentenced packed for {} docs".format(
        len(X), len(sent_lemma_db_list)))
    return X, y, groups
...
def pack_all_doc_sentences_to_map(dir_name, per_par=False, limit=0, doc_as_sequence=0, neighbor_radius=2, sent_lemma_db_list=[], tf_types=['word', 'char_wb'], merged_str="merged_db", tf_suffix=''):
    ...
    for i, doc_name in enumerate(sent_lemma_db_list):
        ...
        if per_par:
            if limit == 0:
                X_doc, y_doc, groups_doc = pack_doc_per_paragraph(
                    doc_idx, doc_as_sequence, neighbor_radius)
            else:
                X_doc, y_doc, groups_doc = pack_doc_per_paragraph_limit(
                    doc_idx, limit, doc_as_sequence, neighbor_radius)
        else:
            X_doc, y_doc, groups_doc = pack_doc_sentences(
                doc_idx, neighbor_radius)
    ...
    print("\n{} items packed for {} docs".format(
        total_sent, len(docs_map.keys())))
    return docs_map
...
def sent2features(sent_idx, idx_in_seq, seq_len=6, neighbor_radius=2, tf_to_use=['lemma','word','char_wb'], tf_features={}):
    ...
    for col in columns:
        if save_feature_value(curr_doc_db['merged'].loc[sent_idx, col], col):
            features["{}".format(
                col)] = curr_doc_db['merged'].loc[sent_idx, col].item()
        ...
        if idx_in_seq > neighbor_dist - 1:
            ...
            for col in columns:
                if save_feature_value(curr_doc_db['merged'].loc[sent_idx-neighbor_dist, col], col):
                    update["-{}:{}".format(neighbor_dist, col)
                           ] = curr_doc_db['merged'].loc[sent_idx-neighbor_dist, col].item()
    ...
    for neighbor_dist in range(1, neighbor_radius+1):
        if idx_in_seq > neighbor_dist - 1:
            update["-{}.sim".format(neighbor_dist)] = curr_doc_db['sim_vec'].iloc[sent_idx,
                                                                                  sent_idx-neighbor_dist].item()
        if idx_in_seq < seq_len - neighbor_dist:
            update["+{}.sim".format(neighbor_dist)] = curr_doc_db['sim_vec'].iloc[sent_idx,
                                                                                  sent_idx+neighbor_dist].item()
...
def sent2features_orig(sent_idx, idx_in_seq, seq_len=6, neighbor_radius=2):
    ...
    for col in columns:
        if save_feature_value(curr_doc_db['merged'].loc[sent_idx, col], col):
            features["{}".format(
                col)] = curr_doc_db['merged'].loc[sent_idx, col]
    if idx_in_seq > 1:
        update = {}
        for col in columns:
            if save_feature_value(curr_doc_db['merged'].loc[sent_idx-1, col], col):
                update["-1:{}".format(col)
                       ] = curr_doc_db['merged'].loc[sent_idx-1, col]
        features.update(update)
    ...
    if idx_in_seq > 2:
        ...
        for col in columns:
            if save_feature_value(curr_doc_db['merged'].loc[sent_idx-2, col], col):
                update["-2:{}".format(col)
                       ] = curr_doc_db['merged'].loc[sent_idx-2, col]
        features.update(update)

    update = {}
    for neighbor_dist in range(1, neighbor_radius+1):
        if idx_in_seq > neighbor_dist - 1:
            update["-{}.sim".format(neighbor_dist)
                   ] = curr_doc_db['sim_vec'].iloc[sent_idx, sent_idx-neighbor_dist]
        if idx_in_seq < seq_len - neighbor_dist:
            update["+{}.sim".format(neighbor_dist)
                   ] = curr_doc_db['sim_vec'].iloc[sent_idx, sent_idx+neighbor_dist]
    ...
    for tf_type in defines.TF_TYPES:
        ...
        if tf_str in curr_doc_db.keys():
            tfidf_feature_indices = curr_doc_db[tf_str][sent_idx, :].nonzero()[
                1]
            for i in tfidf_feature_indices:
                update["{}_{}".format(tf_str, i)
                       ] = curr_doc_db[tf_str][sent_idx, i]
            features.update(update)
...
def get_num_text_union(df):
    ...
    transformer_text = FunctionTransformer(
        lambda x: x['narrative'], validate=False)
    transfomer_numeric = FunctionTransformer(
        lambda x: x[numeric_cols], validate=False)
...
def load_stop_words():
    stop_words = [x.strip()
                  for x in open('heb_stopwords.txt', 'r').read().split('\n')]
    return stop_words
...
def plot_important_features(coef, feature_names, top_n=20, ax=None, rotation=60):
    ...
    heb_feature_names = [bidialg.get_display(
        feature) for feature in feature_names[important]]
...
def get_train_test_text(X, y):
    ...
    print("len train: {}, len test: {}".format(
...
def split_db(_db):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        data, label, stratify=label, random_state=0)
...
def run_model(db):
    ...
    plot_important_features(sgc.coef_.ravel(), np.array(
        tdif.get_feature_names()), top_n=20, rotation=40)
    ax = plt.gca()
    plt.show()
...
def get_cross_val_score_pred(estimator, db, tf, cv_splits, prefix="", tf_types=[],dir_name='',sampler=None):
    ...
    for idx, (split, indices) in enumerate(cv_splits.items()):
        ...
        print("{} split fit of {} samples took {}".format(
            split, len(y_true), time.strftime("%H:%M:%S", time.gmtime(fit_time))))
        pred = estimator.predict(X_test)
        single_cv_db['{}_predicted'.format(prefix)] = pred
        single_cv_db['{}_group'.format(prefix)] = test_group
        single_cv_db['{}_split'.format(prefix)] = int(split)
        single_cv_db['{}_true'.format(prefix)] = y_true
...
def add_score(scores, regressorName, dataType):
    global scores_df
    scores_df.loc[regressorName + '_' +
                  dataType, 'f1'] = scores['test_f1'].mean()
    scores_df.loc[regressorName + '_' + dataType,
...
def cross_val_all_regerssors(my_df, X_train, y_train, groups, feature_set, cv=10):
    ...
    for regr in regressors_type:
        myScorer.get_cross_val_score(
            regr, X_train, y_train, groups, feature_set, cv=cv)
    return pd.concat([myScorer.scores_df, my_df])


def prepared_cross_val_all_regerssors(sent_db, tf, splits, prefix):
    regr_pred = {}
    for regr in regressors_type:
...
#     print ("Test idx: {}..., train idx: {}...".format(train_idx[:10],test_idx[:10]))
    ...
    print("True labels {:.2f} of train, {:.2f} of test".format(
        count_true_labels_ratio(y_train), count_true_labels_ratio(y_test)))
